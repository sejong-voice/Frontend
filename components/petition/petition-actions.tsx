...
export function PetitionActions({
  ...
  const [statusToSubmit, setStatusToSubmit] = useState<"COMPLETED" | "REJECTED">("COMPLETED")
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(statusToSubmit === "COMPLETED" ? "완료 처리가 완료되었습니다." : "반려 처리가 완료되었습니다.")
      ...
      {showAdminActions && (
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button 
              size="sm" 
              className={cn("gap-1.5", statusToSubmit === "COMPLETED" && showResponseForm ? "ring-2 ring-primary ring-offset-2" : "")}
              onClick={() => {
            ...
            <Button
              ...
              className={cn(
                ...
                statusToSubmit === "REJECTED" && showResponseForm ? "ring-2 ring-destructive ring-offset-2" : ""
              )}
              onClick={() => {
          ...
          {showResponseForm && (
            <div className="flex flex-col gap-3 rounded-md border border-border bg-card p-4">
              <p className="text-sm font-medium text-foreground">
                {statusToSubmit === "COMPLETED" ? "완료 처리 - 공식 입장문 작성" : "반려 처리 - 반려 사유 작성"}
              </p>
              <Textarea
